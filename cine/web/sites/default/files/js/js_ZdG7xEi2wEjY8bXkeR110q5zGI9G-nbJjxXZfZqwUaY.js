...
(function ($) {
    ...
    //********************************************************************************
    //HEADER VERIFY IF ADMIN
    //********************************************************************************
    
    //Load admin toolbar in sessionStorage
    let admin = sessionStorage.getItem('admin');

    hbar = $('#toolbar-administration').outerHeight();
    ...
    $(document).ready(function(){
        if(admin == 1){
            ...
            $('#menuPageContainer').height($('#menuPageContainer').height() + 79);
        }
    });
    ...
    //********************************************************************************
        ...
        if (headerID != null){
            ...
            //HEADER GET POSITION PAGE MOUSE
            currentHeight1 = $('#toolbar-bar').outerHeight();
            currentHeight2 = $('.toolbar-menu-administration').outerHeight();
            currentHeight3 = $('header').outerHeight();

            if(currentHeight1 == undefined){
               currentHeight1 = 25;
               currentHeight2 = 25;
            }

            teste = currentHeight1 + currentHeight2;

            $( document ).on( "mousemove", function( event,admin ) {
              //$( "#log" ).text( "pageX: " + event.pageX + ", pageY: " + event.pageY );
              if (event.pageY <= currentHeight3 + 50 + teste ) {
                $('header').addClass('header-on').removeClass('header-off');
              } else if (event.pageY > currentHeight3 + 50 + teste) {
                $('header').addClass('header-off').removeClass('header-on');
              }
    ...
    //********************************************************************************
    //MENU CONSTRUCTOR MENU DIMENSIONS AND RESIZE
    //********************************************************************************

        $(window).on("load resize",function() {

            var intervalo = setInterval(resizeMenuMobile, 50);
            ...
            function resizeMenuMobile() {
                ...
                window.setTimeout(function() {
                    ...
                    //Get height elements parts menu
                    var heightMenuMobile = $('#menuMobile').outerHeight();
                    var heightMenuBanner = $('#menuBanner').outerHeight();
                    var heightMenuColors = $('#menuColors').outerHeight();
                    var heightMenuFooter = $('#menuFooter').outerHeight();

                    var heightMenuItemns = heightMenuBanner + heightMenuColors + heightMenuFooter;
                    var AddittMenuItemns = heightMenuMobile - heightMenuItemns - 100;

                    //Add new height to menu after resize
                    $('#menu').height(AddittMenuItemns);
    ...
    $(window).on("load",function(){
        ...
        btnMenu2 = "#btnMenuMobile";
        btnMenu1 = "#btnMenuPage";

        //Active function and limit clicks in sequence
        $(function() {

            //Active menu desktop
            $(btnMenu1).on('click', function menuDesk() {
                menuPage();

                $(btnMenu1).off('click', menuDesk);
                setTimeout(function() {
                    $(btnMenu1).on('click', menuDesk);
                }, 500);
            });

            //Active menu mobile
            $(btnMenu2).on('click', function menuMobi() {
                menuMobile();

                $(btnMenu2).off('click', menuMobi);
                setTimeout(function() {
                    $(btnMenu2).on('click', menuMobi);
                }, 500);
            });
    ...
    //********************************************************************************
    //COUNT ITENS MENU AND ADD LINK MORE/GO PAGE
    //********************************************************************************

    $('#mnuPaginate .level-one').attr('id', function(i) {
       return 'menu-item'+(i+1);
       var item = 'menu-item'+(i+1);
    });

    var totalDisplay = $('#mnuPaginate .menu').data('myval'); //getter result twig
    var elements = $('.level-one');

    elements.each(function() {

        var itemID = ($(this).attr('id'));
        var itemName =  $("#"+itemID+ " ul li ").length;
        
        if(itemName == totalDisplay) {
            $link = $("#"+itemID).find('a').clone().text('');
            var btnMoreMenu = '<div class="more">...</div>';
            $(btnMoreMenu).appendTo($("#"+itemID));
            $("#"+itemID + ' .more').contents().wrap($link);
        }
    ...
    //var btnMenu = $("#btnMenuPage");
    var checkFlag = true;
    var menuPageCont = "#menuPageContainer";
    var menuPageMenu = "#menuPage";
    ...
    $(window).on('resize', function(){
        ...
        if(checkFlag == false || windowWidth < 1440){
            menuPage();
        }else if (checkFlag == false || windowWidth > 1440){
            $("#menuMobile").removeClass('active');
        }
    ...
    function menuPage() {
        ...
        if(checkFlag == true){
           ...
           setTimeout(function() {
                $(menuPageCont).addClass('on');
                $(menuPageMenu).addClass('on');
                $(menuPageMenu).addClass('active');
            }, 100);

            $(menuPageCont).css('display','block');
            $(menuPageMenu).css('display','block');

            $("body").addClass("showing");
            $("#header").addClass('header-onx');

        }else{

            setTimeout(function() {
                $(menuPageCont).css('display','none');
                $(menuPageMenu).css('display','none');
            }, 200);   

            $(menuPageCont).removeClass('off');
            $(menuPageCont).removeClass('on');

            $(menuPageMenu).removeClass('off');
            $(menuPageMenu).removeClass('on');
            $(menuPageMenu).removeClass('active');

            $("body").removeClass("showing");
            $("#header").removeClass('header-onx');
    ...
    function startSlider(obj, timer) {
        ...
        function moveLeft() {
           ...
           $(id+' ul').css({
            ...
            //$(id+' ul li').prependTo(id+' ul');
            //$(id+' ul li').css('z-index', '1000');
        ...
        function moveRight2() {
            ...
            /* setTimeout( function() {
                $(id+' ul').css({
                    transition: "1s", transform:  "translateX(0vw)"
                });
            }, 100,
            */
            /*setTimeout( function(){
